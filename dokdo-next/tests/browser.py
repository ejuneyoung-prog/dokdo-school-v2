    ...
    for width in [390,768,1440]:
      for lang in ['ko','en']:
        ...
        try:
            ...
            page.click('#start-placement')
            check(prefix+' age gate before placement',page.locator('#age-dialog').is_visible() and page.evaluate('DokdoApp.lesson===null'))
            page.click('#age-form button[type=submit]')
            check(prefix+' missing age cannot start diagnostic',page.locator('#age-dialog').is_visible() and page.evaluate('DokdoApp.state.learningProfile==null'))
            page.fill('#nickname','Learner');page.select_option('#age-band','8-9');page.click('#age-form button[type=submit]')
            page.wait_for_selector('#question-title',state='visible')
            check(prefix+' low-primary pool is actually simple',page.evaluate("DokdoApp.lesson.item.cognitive<=2 && DokdoApp.lesson.order.length>=2 && DokdoApp.lesson.order.length<=4 && DokdoApp.lesson.diagnostic.track==='early'"))
            for _ in range(5):
            ...
            check(prefix+' diagnostic does not invent school grade or invitations',page.evaluate('DokdoApp.state.grade==="K" && DokdoApp.state.gcHit===0 && DokdoApp.state.xp===0'))
