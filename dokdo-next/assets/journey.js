...
'use strict';const LAP=1025,
/* One colour per completed circuit. Planned five deep so a fourth and fifth
 * lap look like their own achievement instead of repeating the third; past
 * the list the last colour carries on rather than throwing. */
COLORS=['#F4C85F','#F5F9FF','#65E0BF','#7FB2FF','#FF9E7A'];
...
function progress(s){const j=s.visual?.journey||{legacy:baseline(s),earned:0};const total=j.legacy+j.earned,laps=Math.floor(total/LAP),part=total%LAP;
 ...
 return {total,legacy:j.legacy,earned:j.earned,laps:wholeLaps,shownLap,filled,remaining:LAP-filled,east,west,markers:Math.max(0,filled-1000),flags:wholeLaps?100:blocks,color:COLORS[Math.min(COLORS.length-1,shownLap-1)],previousColor:COLORS[Math.min(COLORS.length-1,Math.max(0,shownLap-2))]};
}
