...
function fresh(now=Date.now()){
 return ensure(Core.ensure({name:'',nick:'',flag:'KR',school:'',schoolCat:'',recoveryCode:'',introDay:'',grade:'K',xp:0,streak:0,
  gcHit:0,run:0,best:0,started:false,passed:[],badgesEver:[],recent:[],recentG:[],
  lastDay:'',m:{}},now),now);
}
function ensure(s,now=Date.now()){
 Core.ensure(s,now);
 if(s.flag==null)s.flag='KR';if(s.school==null)s.school='';if(s.schoolCat==null)s.schoolCat='';if(s.recoveryCode==null)s.recoveryCode='';if(s.introDay==null)s.introDay='';
